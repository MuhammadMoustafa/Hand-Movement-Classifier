        ...
        if select == '1' :
            while(True):
                try:
                    ...
                    input_test_size = 50
                    select2 = raw_input("Write 1 to shuffle the data\nany thing else to continue without shuffle\n") #use shuffle or not
                    if select2 == '1':
            ...
            while(True):
                try:
                    input1 , input2 = read_data(raw_input('insert the name of file#1 with out the extention .mat\n '), raw_input('insert the name of file#2 with out the extention .mat\n '))    
                    break
                except IOError:
                    print "file not found\nplease make sure that the 2 matlab file are in the same directory that the program on or write the full path instead of the file name\n"
            ...
            while (True):
                input1_fs = raw_input("write the sampling frequency of the first record\n")
                input2_fs = raw_input("write the sampling frequency of the second record\n")
            ...
            while(True):
                temp = raw_input("write the percentage you want to train the model with 'any number between 0 and 100' \n")
                try :  
                    if ( 0< float(temp) <100 ):
